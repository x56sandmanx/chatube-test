...
function CreatePersona() {
  ...
  const [userPersonaTags, setUserPersonaTags] = useState([])

  const [categories, setCategories] = useState([])
  const [userPersonaCategories, setUserPersonaCategories] = useState([])

  const [userPersonaName, setUserPersonaName] = useState('')
  const [userPersonaIsPublic, setUserPersonaIsPublic] = useState(0)
  const [userPersonaDesc, setUserPersonaDesc] = useState('')
  const [userPersonaImage, setUserPersonaImage] = useState(null)
  const [userPersonaPreview, setUserPersonaPreview] = useState()
  const [userPersonaPronouns, setUserPersonaPronouns] = useState('')
  const [userPersonaEmotion, setUserPersonaEmotion] = useState('')

  const [userPersonaNegativePositive, setUserPersonaNegativePositive] = useState(5)
  const [userPersonaAggressivePeaceful, setUserPersonaAggressivePeaceful] = useState(5)
  const [userPersonaCautiousOpen, setUserPersonaCautiousOpen] = useState(5)
  const [userPersonaIntrovertExtravert, setUserPersonaIntrovertExtravert] = useState(5)
  const [userPersonaInsecureConfident, setUserPersonaInsecureConfident] = useState(5)

  const [userPersonaSadnessJoy, setUserPersonaSadnessJoy] = useState(5)
  const [userPersonaAngerFear, setUserPersonaAngerFear] = useState(5)
  const [userPersonaDisgustTrust, setUserPersonaDisgustTrust] = useState(5)
  const [userPersonaAnticipationSurprise, setUserPersonaAnticipationSurprise] = useState(5)
  const [userPersonaEmotionalFluidity, setUserPersonaEmotionalFluidity] = useState(0.0)

  const [userPersonaVoice, setUserPersonaVoice] = useState('')
  const [userPersonaVoicePitch, setUserPersonaVoicePitch] = useState(0.5)
  const [userPersonaTalkingSpeed, setUserPersonaTalkingSpeed] = useState(2.5)
  ...
  const handleSubmitCharacter = () => {
    if(userPersonaTags.length !== 0) {
      setError('Creating Persona. . .')

      const userPersona = {
        userPersonaIsPublic: userPersonaIsPublic,
        userPersonaCreatedBy: LoginCreds.userName,
      ...
      fetch(`${BaseURL}/createUserPersona?devToken=${LoginCreds.devToken}`, {
        method: 'POST',
        body: formData
      })
      .then((res) => res.json())
      .then((data) => {
        if(data.response.error)
          ...
          setError('Persona Created!')
          Characters.userPersonas.push(data.response.userPersona)
          LoginCreds.userPersonaId = data.response.userPersona.userPersonaId
          setTimeout(() => {
            navigate('/')
          }, 2000)
        }
